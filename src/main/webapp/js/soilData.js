 ...
 * @returns
 */
function addpageInfo_soilData(e,page_Info){
		$(page_Info).empty();
		$(page_Info).append("当前第 "+e.data.pageNum+" 页,总共 "
				+e.data.pages+" 页,总共 "
				+e.data.total+" 记录");
	}
	function addPageNav_soilData(e,page_nav){
		//添加翻页导航栏
		$(page_nav).empty();
		...
		if(e.data.hasPreviousPage == false){
			...
			prePageLi.click(function() {
				//**
				showAllSoilData(e.data.pageNum - 1);
			});
